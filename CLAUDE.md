...
5. **Quotes** — a collection of verbatim passages from the source notes that bear on this concept. Can be extensive. Include Gustaf's own words and external authors' words; attribute each clearly. Format: `> "Quote text." — Name, [[Source note]]`. Never paraphrase here — if it is not a direct quote from the source file, it does not belong in this section.
...
- `/ingest` — promote `inbox/<kind>/` items into `sources/<kind>/` with frontmatter, provenance, longform split. Updates `.manifest.json`. **After successfully writing the destination file, delete the original from `inbox/` so the subfolder is left empty.** For consumed articles, the destination filename must be `YYYY-MM-DD-slug.md`, using date precedence `published` → `created` → `captured` → `ingested`; never leave a newly ingested article as a raw title filename. Run `_ai/scripts/check_article_filenames.py` on touched article files before commit. If an inbox item arrives in an unsupported format, treat format handling as part of ingest: inspect the relevant pipeline docs and existing patterns, derive the safest sensible conversion path, and attempt that conversion yourself before declaring the item blocked.
- `/compile` — scan `sources/` for recurring themes; promote to `wiki/concepts/` when 2-source rule fires; otherwise log to `wiki/_candidates.md`. **Before writing any concept file, check if it already exists by listing `wiki/concepts/`. If it exists, update it in place — never write a second copy with a numbered suffix.** Each compiled concept must follow the five-part structure (core claim → evidence → tension → implication → quotes) defined in the Concept structure section. The core claim must be anchored in a `voice` or `thinking` note if one exists — consumed sources frame the claim but don't make it. A concept that only summarizes what external sources say is not ready to be written; log it to `_candidates.md` and note what first-party material is missing. The Quotes section must be populated by reading the actual source files and lifting verbatim passages — do not invent or paraphrase quotes.
